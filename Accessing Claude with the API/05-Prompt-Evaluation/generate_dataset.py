...
def generate_dataset(n=5):
    """Generate an evaluation dataset of AWS-related tasks.

    Uses Claude to generate N diverse tasks that require:
    - Writing a single Python function, or
    - Creating a single JSON config, or
    - Writing a single regex

    All tasks are AWS-specific and avoid large code requirements.

    We use Haiku model here (not the main model) because:
    1. Dataset generation is a "supporting task", not the main evaluation
    2. Haiku is much cheaper (~1/4 the cost of Sonnet)
    3. For generating diverse task descriptions, Haiku quality is sufficient
    4. Speed matters when generating many datasets during iteration
    """
...
- A single Python function (lambda, parser, validator)
- A single JSON config object (EventBridge rule, IAM policy snippet)
...
Return ONLY valid JSON array with format: [{{"task": "description"}}]
